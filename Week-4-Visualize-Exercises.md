...
``` r
# ("mapping =" is not always necessary)
...
ggplot(data = mpg) +
  ...
  labs(x = "Engine Size Displacement",
       ...
       title = "The bigger the engine the less milage we get on the highway") + 
  theme_light()
```
...
# This makes one row, four columns by cyl
q + facet_grid(. ~ cyl)
```
...
``` r
# This makes three rows, one column by drv
q + facet_grid(drv ~ .)
```
...
``` r
# This makes three rows by drv, four columns by cyl
...
``` r
# This makes a graph for each value in class
q + facet_wrap(~ class)
```
...
``` r
# This will add two geometries to one plot and save the graph
